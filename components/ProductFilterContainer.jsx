    ...
    return (
        <>
            <div className="grid grid-cols-1 lg:grid-cols-4 md:grid-cols-2 w-full">
                {data
                    ? data.map((item) => {
                        return <AgeProductCard key={item.id} id={item.id} name={item.attributes.product.data.attributes.name} defaultPrice={item.attributes.product.data.attributes.defaultPrice} coverImage={item.attributes.product.data.attributes.coverImage.data} brand={item.attributes.product.data.attributes.brand} />;
                    })
                    : null}
            </div>
            <AgePagination
